...
> ⚠️ **비활성 상태(2026-08-12)**: 대상 인프라(EC2 `i-025aa470d3d8a127e` 등)가 해체되어
> 워크플로 4종을 전부 수동 비활성화했다(`gh workflow enable <id>`로 재활성 가능).
> 재가동 시에는 aws-application-prod 계정 재구축 + OIDC 전환과 함께 재작성 예정 — `tasks/todo.md` Phase 3.

## 🚀 자동 배포 작동 방법

### 설정 상태 (비활성화 전 기준)

워크플로우가 설정되어 있으며, 다음 조건에서 **자동으로 실행**됩니다:
