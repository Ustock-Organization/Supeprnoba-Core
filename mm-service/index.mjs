/**
 * Supernoba Market Maker Service v6
 *
...
console.log('=== Supernoba Market Maker Service v6 ===');
...
async function startSymbol(symbol) {
  ...
  // Redis에서 설정 읽기
  const configStr = await backupCache.get(`mm:config:${symbol}`);
  const config = configStr ? JSON.parse(configStr) : { basePrice: 100, period: 600, amplitude: 0.1, tickInterval: 1000, tradeQuantity: 10 };
    ...
    switch (cmd.action) {
      ...
      case 'reload':
        ...
        if (cmd.symbol && activeSymbols.has(cmd.symbol)) {
          const configStr = await backupCache.get(`mm:config:${cmd.symbol}`);
          if (configStr) {
            const instance = activeSymbols.get(cmd.symbol);
            instance.config = JSON.parse(configStr);
